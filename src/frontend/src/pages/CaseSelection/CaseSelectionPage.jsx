...
export default function CaseSelectionPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="section-heading border-b border-line pb-6"><div><h1>Processos cíveis</h1><p className="text-xs text-muted mt-3">{currentProfile.organization} · Cada caso, com o contexto que importa</p></div><div className="text-xs text-muted space-y-2"><p><strong className="text-ink">{resource.data?.total ?? '—'}</strong> {resource.data?.total === 1 ? 'processo encontrado' : 'processos encontrados'}</p><p>Exposição desta página: <strong className="text-ink">{resource.loading || resource.error ? '—' : money(cases.reduce((sum, item) => sum + item.claimValue, 0))}</strong></p></div></div>
