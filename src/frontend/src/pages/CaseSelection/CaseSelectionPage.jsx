...
export default function CaseSelectionPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="section-heading border-b border-line pb-6"><div><h1>Processos cíveis</h1><p className="text-xs text-muted mt-3">{currentProfile.organization} · Cada caso, com o contexto que importa</p></div><div className="text-xs text-muted space-y-2"><p><strong className="text-ink">{resource.data?.total ?? '—'}</strong> processos nos filtros da API</p><p>Exposição desta página: <strong className="text-ink">{resource.loading ? '—' : money(cases.reduce((sum, item) => sum + item.claimValue, 0))}</strong></p></div></div>
    {resource.data?.isMock && <p className="notice">Dados de demonstração fornecidos pela API.</p>}
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      <label><span className="field-label">Status do processo</span><select className="field" value={filters.status} onChange={event => changeFilter('status', event.target.value)}><option value="">Todos os status</option><option value="PENDENTE">Pendente</option><option value="EM_ANALISE">Em análise</option><option value="CONCLUIDO">Concluído</option></select></label>
    ...
    <div className="grid md:grid-cols-3 gap-3">
      ...
      <label><span className="field-label">Recomendação nesta página</span><select className="field" value={recommendation} onChange={event => setRecommendation(event.target.value)}><option value="">Todas</option><option value="DEFESA">Defesa</option><option value="ACORDO">Acordo</option><option value="NONE">Sem recomendação</option></select></label>
