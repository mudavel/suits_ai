...
export default function DraftsPanel({ caseId }) {
  ...
  const selectDraft = value => { setDraft(value); setContent(value.content_markdown); setExported(false) }
  ...
  return <section id="minutas" className="panel space-y-5 scroll-mt-6">
    <div className="section-heading"><div><p className="eyebrow">GERAÇÃO E REVISÃO</p><h2>Minutas</h2></div><button type="button" className="button-secondary" onClick={drafts.reload} disabled={operation.pending || drafts.loading}>Atualizar histórico</button></div>
    <form onSubmit={generate} className="space-y-4">
      ...
      </div><button className="button-primary" disabled={operation.pending || dirty}>Gerar minuta</button>
    </form>
    {operation.pending && <Busy>Processando a minuta...</Busy>}<ErrorNotice error={operation.error} />
    ...
    {draft && <div className="space-y-4 border-t border-line pt-5">
      ...
      <p className="notice">Revisão necessária antes do uso. As edições abaixo entram no PDF exportado; a versão original permanece salva no histórico. Edições ainda não exportadas ficam apenas nesta tela.</p>
      <label className="block"><span className="field-label">Texto da minuta (Markdown)</span><textarea className="field draft-editor" aria-label="Texto da minuta" maxLength={60000} value={content} onChange={event => { setContent(event.target.value); setExported(false) }} disabled={operation.pending} /></label>
      <div className="flex flex-wrap gap-3 items-center"><button type="button" className="button-primary" disabled={operation.pending || !content.trim()} onClick={download}><Download size={15} />Exportar PDF revisado</button>{dirty && <button type="button" className="button-secondary" disabled={operation.pending} onClick={() => { setContent(draft.content_markdown); setExported(false) }}>Descartar edição</button>}<span role="status" className="text-xs text-muted">{exported ? 'PDF exportado com o texto atual.' : dirty ? 'Texto editado nesta sessão.' : ''}</span></div>
