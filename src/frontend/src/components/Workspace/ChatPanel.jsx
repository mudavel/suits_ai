...
import { submitChatOnEnter } from '../../services/chatKeyboard'
...
function Conversation({ caseId, sessionId, onSending, onSaved, onResponse }) {
  ...
  return <div className="space-y-4">
    ...
    <form onSubmit={submit} className="space-y-2"><label className="field-label" htmlFor="chat-message">Mensagem ao copiloto</label><textarea id="chat-message" className="field min-h-24" maxLength={4000} value={message} onChange={event => setMessage(event.target.value)} onKeyDown={submitChatOnEnter} aria-describedby="chat-keyboard-help" disabled={operation.pending} placeholder="Quais documentos sustentam a defesa?" required /><p id="chat-keyboard-help" className="text-[10px] text-muted">Enter para enviar · Shift+Enter para nova linha</p><div className="flex items-center justify-between gap-3"><span className="text-[10px] text-muted">{message.length}/4.000 · últimas 20 mensagens</span><button className="button-primary" disabled={operation.pending || history.loading || !!history.error || !message.trim()}><Send size={14} />Enviar</button></div></form>
  </div>
}
